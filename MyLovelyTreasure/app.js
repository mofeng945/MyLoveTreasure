...
function initApp() {
    // 检测设备类型
    isMobileDevice = window.innerWidth < 768 || /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) && ('ontouchstart' in window || navigator.maxTouchPoints > 0);
...
// 在窗口大小变化时重新初始化移动端UI
function reinitMobileUI(isMobile) {
    ...
    if (isMobileDevice) {
        ...
        // 如果切换到桌面设备模式，可以在这里添加清理移动设备特定UI的代码
        console.log('Switching to desktop UI...');
    }
}

// 重新初始化移动端交互
function reinitMobileInteractions() {
    // 分类选择器是通过<select>元素实现的，确保在移动设备上正常工作
    const mainCategorySelect = document.getElementById('main-category');
    const subCategorySelect = document.getElementById('sub-category');
    
    if (mainCategorySelect) {
        // 移除旧事件监听器
        const newMainSelect = mainCategorySelect.cloneNode(true);
        mainCategorySelect.parentNode.replaceChild(newMainSelect, mainCategorySelect);
        
        // 重新绑定change事件
        newMainSelect.addEventListener('change', updateSubCategories);
        
        // 对于移动设备上的<select>元素，添加适当的触摸优化样式
        newMainSelect.style.touchAction = 'manipulation';
    }
    
    if (subCategorySelect) {
        // 移除旧事件监听器
        const newSubSelect = subCategorySelect.cloneNode(true);
        subCategorySelect.parentNode.replaceChild(newSubSelect, subCategorySelect);
        
        // 对于移动设备上的<select>元素，添加适当的触摸优化样式
        newSubSelect.style.touchAction = 'manipulation';
    }
    
    // 重新绑定所有按钮事件
    const buttons = document.querySelectorAll('button, .btn, .add-record-btn');
    buttons.forEach(button => {
        if (button.id === 'add-record-btn') return; // 特殊处理
...
// 确保移动端标签页正确显示
function ensureMobileTabsVisible() {
    // 确保移动设备上只显示一个标签页内容
...
function optimizeMobileTouchEvents() {
    // 禁用移动端双击缩放，但保留单指触摸的默认行为
    document.addEventListener('touchstart', function(e) {
        // 只在多点触摸时阻止默认行为，避免影响单指操作
